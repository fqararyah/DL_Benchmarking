...
def conv2d(
    ...
    with tf.compat.v1.variable_scope('conv'):
        ...
        initializer = param_initializer.get(
            init_key, tf.keras.initializers.variance_scaling())
...
def depthwise_conv2d(
    ...
    with tf.compat.v1.variable_scope('conv'):
        init_key = '%s/weight' % tf.compat.v1.get_variable_scope().name
        initializer = param_initializer.get(
            init_key, tf.compat.v1.variance_scaling_initializer())
        weight = tf.compat.v1.get_variable(
...
def fc_layer(_input, out_units, use_bias=False, param_initializer=None):
    ...
    with tf.compat.v1.variable_scope('linear'):
        init_key = '%s/weight' % tf.compat.v1.get_variable_scope().name
        initializer = param_initializer.get(
            init_key, tf.keras.initializers.glorot_normal())
        weight = tf.compat.v1.get_variable(
...
def batch_norm(
    ...
    with tf.compat.v1.variable_scope('bn'):
        ...
        output = tf.compat.v1.layers.batch_normalization(
            ...
            beta_initializer=bn_init['beta'],
            gamma_initializer=bn_init['gamma'],
            moving_mean_initializer=bn_init['moving_mean'],
            moving_variance_initializer=bn_init['moving_variance'],
            epsilon=epsilon,
        )
    return output
