...
if layer_type != 'pw':
    if conv_strides == 1:
        ifms = np.pad(ifms, ((0,0),(padding_val,padding_val),(padding_val,padding_val)), mode='constant')
    elif conv_strides == 2:
        ifms = np.pad(ifms, ((0,0),(0,padding_val),(0,padding_val)), mode='constant')
    ...
    for i in range(layers_weights_shape[layer_index].num_of_filters):
        for j in range(ofms_shape[1]):
            for k in range(ofms_shape[2]):
                ...
                tmp = tmp * layers_scale_ifms[layer_index] * layers_scale_weights[layer_index][0] / \
                    layers_scale_ofms[layer_index] + layers_ofms_zero_point[layer_index]
                if tmp > 0:
                    tmp = int(tmp + 0.5)
                else:
                    tmp = int(tmp -0.5)
