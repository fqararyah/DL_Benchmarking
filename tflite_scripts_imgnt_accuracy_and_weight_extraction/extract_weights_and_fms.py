...
def is_it_bias(tensor_details):
    is_it = interpreter.get_tensor(t['index']).size == layers_weights[last_assigned_layer].num_of_filters
    return is_it
...
for t in interpreter.get_tensor_details():
    ...
    if index_in_weights >= 0:
        ...
        np.savetxt('./weights/weights_' + str(index_in_weights) + '_scales.txt', t['quantization_parameters']['scales'])
        np.savetxt('./weights/weights_' + str(index_in_weights) + '_zero_points.txt', t['quantization_parameters']['zero_points'], fmt='%i')
        ...
        np.savetxt('fms/fms_' + str(fms_count) + '_scales.txt', t['quantization_parameters']['scales'])
        np.savetxt('fms/fms_' + str(fms_count) + '_zero_points.txt', t['quantization_parameters']['zero_points'], fmt='%i')
        # with open('./fms/fms_' + str(fms_count) + '_quantization_parameters.txt', 'w') as f:
        #     f.write(str(t['quantization_parameters']))
        fms_count += 1
    elif is_it_bias(t):
        np.savetxt('./weights/weights_' + str(last_assigned_layer) + '_biases.txt', current_tensor, fmt='%i')
