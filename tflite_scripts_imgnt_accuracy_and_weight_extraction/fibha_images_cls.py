...
print(len(fibha_images))
limit = len(fibha_images)
i=-1
processed = 0
while processed < limit:
    img_name = DATA_PATH + '/' + fibha_image_names[processed]
    a_test_image = load_img(img_name, target_size=(224, 224))
    numpy_image = img_to_array(a_test_image)
    image_batch = np.expand_dims(numpy_image, axis=0)
    ...
    top5 = np.flip(top5)
    prediction_dict = {"dets":top5.tolist(), "image": fibha_image_names[processed]}
